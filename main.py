
from tkinter import *
import tkinter as tk
from tkinter import ttk
from PIL import Image,ImageTk

root=Tk()
root.title("BMI Calculator")
root.geometry("500x600+300+200")
root.resizable(False,False)
root.configure(bg="light green")

def BMI():
    h=float(Height.get())
    w=float(Weight.get())

    #height conversion in meter
    m=h/100
    bmi=round(float(w/m**2),1)
    label1.config(text=bmi)

    if bmi<=18.5:
        label2.config(text="Underweight!")
        label3.config(text='You are underweight,\n Lead to nutrient deficiencies,weakened \n immune system, and other health issues.')
    elif bmi>18.5 and bmi<=25:
        label2.config(text="Normal!")
        label3.config(text='You are Healthy!!')
    elif bmi>25 and bmi<=30:
        label2.config(text="Overweight!")
        label3.config(text='The person is \n slightly overweight! \n You Want to take Doctors advice to lose some wight ')
    else:
        label2.config(text="Obes!")
        label3.config(text='Health may be at risk,if they do not \n lose weight! ')
                            
        
            

#photo icon
image_icon=PhotoImage(file="images/icons8-healthy-food-calories-calculator-48.png")
root.iconphoto(False,image_icon)

#top
top=PhotoImage(file="images/top.png")
top_image=Label(root,image=top,background="#f0f1f5")
top_image.place(x=-20,y=-10)

#bottom box
Label(root,width=72,height=18,bg="light blue").pack(side=BOTTOM)
#two box
box=PhotoImage(file="images/box.png")
Label(root,image=box).place(x=20,y=100)
Label(root,image=box).place(x=240,y=100)

#scale
scale=PhotoImage(file="images/scale.png")
Label(root,image=scale,bg="white").place(x=20,y=327)

#############Slider1############
current_value = tk.DoubleVar()

def get_current_value():
    return '{: .2f}'.format(current_value.get())

def slider_changed(event):
    Height.set(get_current_value())

    size=int(float(get_current_value()))
    img= (Image.open("images/men-suit-png-8.png"))
    resized_image=img.resize((50,10+size))
    photo2=ImageTk.PhotoImage(resized_image)
    secondimage.config(image=photo2)
    secondimage.place(x=70,y=530-size)
    secondimage.image=photo2
             
             
#Color change Command for Scale
style = ttk.Style()
style.configure("TScale",background="White")
slider= ttk.Scale(root,from_=0, to=220,orient='horizontal',style="TScale",command=slider_changed,variable=current_value)
slider.place(x=80,y=250)


###############################





##@@@@@@@@@@@@@@@@Slider2@@@@@@@@@@@@

current_value2 = tk.DoubleVar()

def get_current_value2():
    return '{: .2f}'.format(current_value2.get())

def slider_changed2(event):
    Weight.set(get_current_value2())
  
#Color change Command for Scale
style2 = ttk.Style()
style2.configure("TScale",background="White")
slider2= ttk.Scale(root,from_=0, to=200,orient='horizontal',style="TScale",command=slider_changed2,variable=current_value2)
slider2.place(x=300,y=250)




#EntryBOX
Height=StringVar()
Weight=StringVar()
height=Entry(root,textvariable=Height,width=5,font="arial 50",bg="#fff",fg="#000",bd=0,justify=CENTER)
height.place(x=35,y=160)
Height.set(get_current_value())

weight=Entry(root,textvariable=Weight,width=5,font="arial 50",bg="#fff",fg="#000",bd=0,justify=CENTER)
weight.place(x=255,y=160)
Weight.set(get_current_value2())

#Man Image
secondimage=Label(root,bg='light blue')
secondimage.place(x=70,y=530)

Button(root,text="view Report",width=15,height=2,font='arial 10 bold',bg="#1f6e68",fg="white",command=BMI).place(x=280,y=340)

label1=Label(root,font='arial 60 bold',bg="light blue",fg="#fff")
label1.place(x=125,y=305)

label2=Label(root,font='arial 20 bold',bg="light blue",fg="#fff")
label2.place(x=280,y=430)

label3=Label(root,font='arial 10 bold',bg="light blue")
label3.place(x=200,y=500)





root.mainloop()
